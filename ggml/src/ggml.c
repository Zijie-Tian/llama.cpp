...
static const struct ggml_type_traits type_traits[GGML_TYPE_COUNT] = {
    ...
    [GGML_TYPE_QLUTATTN_K1_128x128] = {
        ...
        .from_float_ref           = (ggml_from_float_t) quantize_block_qlutattn_k1_128x128_ref,
    },
    [GGML_TYPE_QLUTATTN_K2_128x128] = {
        ...
        .from_float_ref           = (ggml_from_float_t) quantize_block_qlutattn_k2_128x128_ref,
    },
    [GGML_TYPE_QLUTATTN_K4_128x128] = {
        ...
        .from_float_ref           = (ggml_from_float_t) quantize_block_qlutattn_k4_128x128_ref,
    },
    [GGML_TYPE_QLUTATTN_V1_128x128] = {
        ...
        .is_quantized             = true,
        .to_float                 = (ggml_to_float_t) dequantize_row_qlutattn_w1g128,   //> Fake
        .from_float_ref           = (ggml_from_float_t) quantize_block_qlutattn_v1_128x128_ref,
    },
    [GGML_TYPE_QLUTATTN_V2_128x128] = {
        ...
        .is_quantized             = true,
        .to_float                 = (ggml_to_float_t) dequantize_row_qlutattn_w2g128,   //> Fake
        .from_float_ref           = (ggml_from_float_t) quantize_block_qlutattn_v2_128x128_ref,
    },
    [GGML_TYPE_QLUTATTN_V4_128x128] = {
        ...
        .is_quantized             = true,
        .to_float                 = (ggml_to_float_t) dequantize_row_qlutattn_w4g128,   //> Fake
        .from_float_ref           = (ggml_from_float_t) quantize_block_qlutattn_v4_128x128_ref,
    },
...
static const char * GGML_UNARY_OP_NAME[GGML_UNARY_OP_COUNT] = {
    ...
    "GELU", "GELU_QUICK", "SILU", "HARDSWISH", "HARDSIGMOID", "EXP", "GELU_ERF",
};
...
static struct ggml_tensor * ggml_new_tensor_impl(struct ggml_context * ctx, enum ggml_type type, int n_dims,
    ...
    *result = (struct ggml_tensor) {
    ...
    result->nb[1] =
        result->nb[0] * (result->ne[0] / ggml_blck_size(type));  //> sizeof(block) * (last_dim / (group_size))
...
struct ggml_tensor * ggml_flash_attn_mixed(struct ggml_context * ctx, struct ggml_tensor * q, struct ggml_tensor * k,
                                           struct ggml_tensor * v, struct ggml_tensor * mask,
                                           struct ggml_tensor * k_quant, struct ggml_tensor * v_quant,
                                           struct ggml_tensor * qk_mask_quant, float scale, float max_bias,
                                           float logit_softcap) {
    GGML_ASSERT(ggml_can_mul_mat(k, q));
    // TODO: check if vT can be multiplied by (k*qT)
    ...
    // permute(0, 2, 1, 3)
    int64_t              ne[4]  = { PACK_SIZE, q->ne[2], q->ne[1], q->ne[3] };
    struct ggml_tensor * result = ggml_new_tensor(ctx, GGML_TYPE_F32, 4, ne);
...
struct ggml_tensor * ggml_flash_attn_ext_with_state(struct ggml_context * ctx, struct ggml_tensor * q,
                                                    struct ggml_tensor * k, struct ggml_tensor * v,
                                                    struct ggml_tensor * mask, struct ggml_tensor * k_quant,
                                                    struct ggml_tensor * v_quant, struct ggml_tensor * qk_mask_quant,
