...
static inline bool is_tmac_2bit_type(enum ggml_type type) {
    return (type == GGML_TYPE_TMAC_BN_0 || type == GGML_TYPE_TMAC_W2G64_0 || type == GGML_TYPE_TMAC_W2G64_1 ||
            type == GGML_TYPE_TMAC_W2G128_0 || type == GGML_TYPE_TMAC_W2G128_1);
}

static inline bool is_tmac_4bit_type(enum ggml_type type) {
    return (type == GGML_TYPE_TMAC_W4G64_0 || type == GGML_TYPE_TMAC_W4G64_1 || type == GGML_TYPE_TMAC_W4G128_0 ||
            type == GGML_TYPE_TMAC_W4G128_1);
}

bool is_tmac_type(enum ggml_type type) {
    return (is_tmac_2bit_type(type) || is_tmac_4bit_type(type));
}

bool is_type_supported(enum ggml_type type) {
    return (type == GGML_TYPE_Q4_0 || type == GGML_TYPE_TQ1_0 || type == GGML_TYPE_TQ2_0 || is_tmac_2bit_type(type) ||
...
static void ggml_tmac_tune_single_kernel_config(const struct tmac_run_single_kernel_settings * const settings,
                                                double &                                             elapsed_time) {
    ...
    GGML_LOG_DEBUG("Run single kernel config: M=%d, N=%d, K=%d, bm=%d, kfactor=%d, actk=%d\n", M, N, K, bm,
                   kernel_config->kfactor, kernel_config->actk);
    int n_try = 0;
    while (total_elapsed.count() < test_time_ms / 1000.0) {
        ...
        auto start = std::chrono::high_resolution_clock::now();
        ggml_tmac_forward_mul_mat(A, B, C, QLUT, LUT_Scales, LUT_Biases, Scales, M, N, K, kernel_config);
        auto end = std::chrono::high_resolution_clock::now();
    ...
    for (int bm : bms) {
        ...
        for (int n : bns) {
            ...
            for (int kfactor : kfactors) {
                ...
                GGML_LOG_INFO(
                    "Tuned kernel config: M=%d, N=%d, K=%d, bm=%d, n=%d, kfactor=%d, bits=%d, g=%d, "
                    "ngroups_per_elem=%d, q_group_size=%d, act_group_size=%d\t TIME: %.4f ms\n",
                    M, N, K, bm, n, kfactor, bits, kernel_config.g, kernel_config.ngroups_per_elem,
                    kernel_config.q_group_size, kernel_config.act_group_size, this_time);
...
static inline void ggml_tmac_transform_tensor(struct ggml_tensor * tensor, const void * origin_data) {
    ...
    GGML_LOG_DEBUG(
        "kcfg (bm=%d, simd_n_in=%d, simd_n_out=%d, kfactor=%d, group_size=%d, lut_scales_size=%d, scales_size=%d, "
        "n_tile_num=%d)\n",
    ...
    if (bm == 0) {
        if (!strcmp(tensor->name, "token_embd.weight") || !strcmp(tensor->name, "output.weight")) {
            GGML_LOG_WARN(
                "Do not find kcfg for %s. Consider compiling T-MAC kernel for it if vocab size is a multiply of 128 or "
                "320, detected %lld.\n",
    ...
    struct tmac_tensor_extra * cur_tensor_extra = new tmac_tensor_extra({ /* .lut_scales_size = */ lut_scales_size,
...
#    ifndef TMAC_EMPTY_WEIGHTS
        ...
        for (int im = 0; im < m / bits; im++) {
            for (int ib = 0; ib < bits; ib++) {
                for (int ik = 0; ik < k / g; ik++) {
                    ...
                    // w = w.reshape(M // bits // simd_n_out, simd_n_out, bits, K // g).transpose(0, 2, 1, 3)
                    int new_idx =
                        new_im * bits * simd_n_out * k / g + new_ib * simd_n_out * k / g + new_isno * k / g + new_ik;
                    // w = w.reshape(M // mgroup, ngroups_per_elem, simd_n_in, K // g).transpose(0, 2, 1, 3)
                    int nb2      = k / g;
        ...
        const float * int_n_scales =
            (const float *) ((const uint8_t *) origin_data + k * m / 8);  // NOTE : K * M / (8 / bits)
...
void ggml_tmac_mul_mat_task_init(void * src1, void * qlut, void * lut_scales, void * lut_biases, int n, int k, int m,
                                 int bits) {
    struct tmac_kernel_config * kernel_config = find_tmac_kernel_config(n, k, bits);
    if (kernel_config == nullptr) {
...
void ggml_tmac_mul_mat_task_compute(void * src0, void * scales, void * qlut, void * lut_scales, void * lut_biases,
                                    void * dst, int n, int k, int m, int bits) {
    struct tmac_kernel_config * kernel_config = find_tmac_kernel_config(n, k, bits);
    if (kernel_config == nullptr) {
        GGML_LOG_INFO("Failed to find kernel config for m%d_k%d_b%d\n", n, k, bits);
        throw std::runtime_error("ggml_tmac_mul_mat_task_compute: Failed to find kernel config for m" +
...
void ggml_backend_tmac_mul_mat(const struct ggml_compute_params * params, struct ggml_tensor * dst) {
    ...
    struct tmac_tensor_extra * wt =
        ((struct ggml::cpu::tmac::tensor_traits *) src0->extra)->get_tmac_tensor_extra(src0->name);
    ...
    for (int ine11 = ith; ine11 < ne11; ine11 += nth) {
        if (sizeof(tmac_float_type) == 2) {
            ...
            ggml_fp32_to_fp16_row((const float *) src1->data + ne10 * ine11, (ggml_fp16_t *) act_input + ne10 * ine11,
                                  ne10);
        }
        ggml_tmac_mul_mat_task_init(act_input + ne10 * ine11, qlut + ne10 * ine11 * 4,
                                    lut_scales + wt->lut_scales_size * ine11, lut_biases + wt->lut_scales_size * ine11,
                                    ne01, ne00, 1, bits);
    }
    ...
    while (current_chunk < nchunk0 * nchunk1) {
        ...
        for (int64_t ichunk0 = ir0_start / chunk_size0; ichunk0 < ir0_end / chunk_size0; ichunk0++) {
            ...
            for (int64_t ine11 = ir1_start; ine11 < ir1_end; ine11++) {
                ...
                ggml_tmac_mul_mat_task_compute(wt->qweights + w_offset, wt->scales + scales_offset, qlut + qlut_offset,
                                               lut_scales + lut_scales_offset, lut_biases + lut_scales_offset,
                                               act_output + dst_offset, ne01, ne00, 1, bits);
                if (sizeof(tmac_float_type) == 2) {
                    ggml_fp16_to_fp32_row((const ggml_fp16_t *) act_output + dst_offset,
                                          (float *) dst->data + dst_offset, chunk_size0);
                }
