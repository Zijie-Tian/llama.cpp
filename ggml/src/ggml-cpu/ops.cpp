 ...
 * @param dst
 */
static void ggml_compute_forward_dup_f16(const ggml_compute_params * params, ggml_tensor * dst) {
    const ggml_tensor * src0 = dst->src[0];
...
static void repack_qlutattn_kv4_128x128(const uint8_t * src, uint8_t * dst, int n_elements) {}

//> ===================================================================================================
//> Helper functions for QLUTATTN quantization and packing
//> ===================================================================================================

// NOTE: Structure for packing parameters
struct qlutattn_pack_params {
    int simd_n_out;
    int simd_n_in;
...
static void qlutattn_pack_weights(const uint8_t * qweight_ptr, uint8_t * qweights_out,
    ...
    //> ===================================================================================================
    // EXPLAIN: Extract individual bits and reorganize them into groups of g for LUT processing
    memset(repack_ws, 0, m * k / g);
    for (int im = 0; im < m / bits; im++) {
        for (int ik = 0; ik < k; ik++) {
            uint8_t v;
            // NOTE: Extract quantized value based on bit width
            if (bits == 1) {
    ...
    //> ===================================================================================================
    // EXPLAIN: Complex permutation for SIMD and cache optimization
    // Implements multi-stage reshape and transpose operations:
...
static void qlutattn_pack_scales(const float * scale_ptr, const float * zero_ptr, ggml_fp16_t * scales_out, int m,
    ...
    if (scales_size < m / bits) {
        ...
        // TODO: Move this if-else outside the loop for better performance
        for (int im = 0; im < m / bits; im++) {
...
static void ggml_compute_forward_dup_f16_qlutattn(const ggml_compute_params * params, ggml_tensor * dst) {
    ...
    const int dh  = (n_kv_heads * ne01 + nth - 1) / nth;
    const int ih0 = dh * ith;
    const int ih1 = MIN(ih0 + dh, n_kv_heads * ne01);

    if (ih0 >= n_kv_heads * ne01 || ih0 >= ih1) {
        return;
    }

    const int bits = get_quant_nbits(dst->type);
    GGML_ASSERT(bits > 0 && "Invalid quantization type");
    const int nelem_per_byte = 8 / bits;

#ifdef GGML_USE_QLUTATTN
    switch (dst->type) {
        // Key tensors: per-token quantization, no transpose
        case GGML_TYPE_QLUTATTN_K1_128x128:
        case GGML_TYPE_QLUTATTN_K2_128x128:
        case GGML_TYPE_QLUTATTN_K4_128x128:
            {
...
static void ggml_compute_forward_dup_f32(const ggml_compute_params * params, ggml_tensor * dst) {
    const ggml_tensor * src0 = dst->src[0];
...
static void ggml_compute_forward_dup_q(const ggml_compute_params * params, ggml_tensor * dst) {
    const ggml_tensor * src0 = dst->src[0];
    ...
    const ggml_to_float_t dequantize_row_q = ggml_get_type_traits(type)->to_float;
    ...
    for (int64_t ir = ir0; ir < ir1; ++ir) {
        ...
        const int64_t x_offset =
            (i00 / qk) * nb00 + i01 * nb01 + i02 * nb02 + i03 * nb03;  //> Offset into source tensor
        ...
        dequantize_row_q((const void *) ((char *) src0->data + x_offset), (float *) ((char *) dst->data + dst_offset),
                         qk);
    }
}

static void ggml_compute_forward_dup_qlutattn(const ggml_compute_params * params, ggml_tensor * dst) {
    const ggml_tensor * src0 = dst->src[0];
    const ggml_tensor * src1 = dst->src[1];
    ...
    for (int i03 = 0; i03 < ne03; i03++) {          // batch
        for (int i02 = 0; i02 < ne02; i02++) {      // head
            for (int ig = 0; ig < n_steps; ig++) {  // quantization group
                ...
                dequantize_row_q((const void *) ((char *) src0->data + src_offset), src0_f32, qk * ne00);

                if (src0->type == GGML_TYPE_QLUTATTN_W4G128_PC || src0->type == GGML_TYPE_QLUTATTN_W2G128_PC ||
                    src0->type == GGML_TYPE_QLUTATTN_W1G128_PC) {
                ...
                } else if (src0->type == GGML_TYPE_QLUTATTN_W4G128_PT || src0->type == GGML_TYPE_QLUTATTN_W2G128_PT ||
                           src0->type == GGML_TYPE_QLUTATTN_W1G128_PT) {
                    // Transpose back from per-channel layout to original layout
...
static void ggml_compute_forward_add_q_f32(const ggml_compute_params * params, ggml_tensor * dst) {
    ...
    const ggml_to_float_t   dequantize_row_q = ggml_get_type_traits(type)->to_float;
    const ggml_from_float_t quantize_row_q   = ggml_get_type_traits_cpu(dtype)->from_float;
...
static void ggml_compute_forward_add1_f32(const ggml_compute_params * params, ggml_tensor * dst) {
    ...
    for (int ir = ir0; ir < ir1; ++ir) {
        ...
        vDSP_vadd((float *) ((char *) src0->data + i3 * nb03 + i2 * nb02 + i1 * nb01), 1,
                  (float *) ((char *) src1->data), 0, (float *) ((char *) dst->data + i3 * nb3 + i2 * nb2 + i1 * nb1),
                  1, ne0);
#else
...
static void ggml_compute_forward_add1_q_f32(const ggml_compute_params * params, ggml_tensor * dst) {
    ...
    const ggml_type         type             = src0->type;
    const ggml_to_float_t   dequantize_row_q = ggml_get_type_traits(type)->to_float;
    const ggml_from_float_t quantize_row_q   = ggml_get_type_traits_cpu(type)->from_float;
    ...
    for (int i3 = 0; i3 < ne3; i3++) {
        for (int i2 = ith; i2 < ne2; i2 += nth) {
            for (int i1 = 0; i1 < ne1; i1++) {
                for (int i0 = 0; i0 < ne0; i0++) {
                    if (i0 < ne00 && i1 < ne01 && i2 < ne02 && i3 < ne03) {
                        x = (const int8_t *) ((const char *) src0->data + (i0) *nb00 + (i1) *nb01 + (i2) *nb02 +
                                              (i3) *nb03);
                    } else {
    ...
    for (int i3 = 0; i3 < ne3; i3++) {
        for (int i2 = ith; i2 < ne2; i2 += nth) {
            for (int i1 = 0; i1 < ne1; i1++) {
                for (int i0 = 0; i0 < ne0; i0++) {
                    if (i0 < ne00 && i1 < ne01 && i2 < ne02 && i3 < ne03) {
                        x = (const float *) ((const char *) src0->data + (i0) *nb00 + (i1) *nb01 + (i2) *nb02 +
                                             (i3) *nb03);
                    } else {
...
static void ggml_compute_forward_gelu_f16(const ggml_compute_params * params, ggml_tensor * dst) {
    ...
    for (int i1 = ir0; i1 < ir1; i1++) {
        ggml_vec_gelu_f16(nc, (ggml_fp16_t *) ((char *) dst->data + i1 * (dst->nb[1])),
...
static void ggml_compute_forward_gelu_erf_f32(const ggml_compute_params * params, ggml_tensor * dst) {
    ...
    for (int i1 = ir0; i1 < ir1; i1++) {
        ggml_vec_gelu_erf_f32(nc, (float *) ((char *) dst->data + i1 * (dst->nb[1])),
                              (float *) ((char *) src0->data + i1 * (src0->nb[1])));
...
static void ggml_compute_forward_gelu_erf_f16(const ggml_compute_params * params, ggml_tensor * dst) {
    ...
    for (int i1 = ir0; i1 < ir1; i1++) {
        ggml_vec_gelu_erf_f16(nc, (ggml_fp16_t *) ((char *) dst->data + i1 * (dst->nb[1])),
                              (ggml_fp16_t *) ((char *) src0->data + i1 * (src0->nb[1])));
...
static void ggml_compute_forward_gelu_quick_f16(const ggml_compute_params * params, ggml_tensor * dst) {
    ...
    for (int i1 = ir0; i1 < ir1; i1++) {
        ggml_vec_gelu_quick_f16(nc, (ggml_fp16_t *) ((char *) dst->data + i1 * (dst->nb[1])),
                                (ggml_fp16_t *) ((char *) src0->data + i1 * (src0->nb[1])));
...
static void ggml_compute_forward_silu_f16(const ggml_compute_params * params, ggml_tensor * dst) {
    ...
    for (int i1 = ir0; i1 < ir1; i1++) {
        ggml_vec_silu_f16(nc, (ggml_fp16_t *) ((char *) dst->data + i1 * (dst->nb[1])),
                          (ggml_fp16_t *) ((char *) src0->data + i1 * (src0->nb[1])));
...
void ggml_compute_forward_reshape(const ggml_compute_params * params, ggml_tensor * dst) {
    // NOP
    GGML_UNUSED(params);
    GGML_UNUSED(dst);
}
...
void ggml_compute_forward_view(const ggml_compute_params * params, ggml_tensor * dst) {
    // NOP
    GGML_UNUSED(params);
    GGML_UNUSED(dst);
}
...
void ggml_compute_forward_permute(const ggml_compute_params * params, ggml_tensor * dst) {
    // NOP
    GGML_UNUSED(params);
    GGML_UNUSED(dst);
}
...
void ggml_compute_forward_transpose(const ggml_compute_params * params, ggml_tensor * dst) {
    // NOP
    GGML_UNUSED(params);
    GGML_UNUSED(dst);
}
...
static void ggml_compute_forward_diag_mask_f32(const ggml_compute_params * params, ggml_tensor * dst,
                                               const float value) {
    const ggml_tensor * src0 = dst->src[0];
...
static void ggml_compute_forward_rope_f32(const ggml_compute_params * params, ggml_tensor * dst, const bool forward) {
    const ggml_tensor * src0 = dst->src[0];
    const ggml_tensor * src1 = dst->src[1];
                ...
                if (is_neox || is_mrope) {
                    if (is_vision) {
                        for (int64_t i0 = 0; i0 < n_dims; i0 += 2) {
                            ...
                            float * dst_data =
                                (float *) ((char *) dst->data + i3 * nb3 + i2 * nb2 + i1 * nb1 + ic * nb0);
...
static void ggml_compute_forward_rope_f16(const ggml_compute_params * params, ggml_tensor * dst, const bool forward) {
    const ggml_tensor * src0 = dst->src[0];
    const ggml_tensor * src1 = dst->src[1];
    const ggml_tensor * src2 = dst->src[2];
...
static void ggml_compute_forward_pool_1d_sk_p0(const ggml_compute_params * params, const ggml_op_pool op, const int k,
                                               ggml_tensor * dst) {
            ...
            for (int64_t i2 = ith; i2 < ne2; i2 += nth) {
                ...
                for (int64_t i1 = 0; i1 < ne1; i1++) {
                    ...
                    for (int64_t i0 = 0; i0 < ne0; i0++) {
                        ...
                        const float a = *(const float *) ((const char *) src0->data + x0 * nb00 + y0 * nb01 +
                                                          i02 * nb02 + i03 * nb03);
                        const float b = *(const float *) ((const char *) src0->data + x1 * nb00 + y0 * nb01 +
                                                          i02 * nb02 + i03 * nb03);
                        const float c = *(const float *) ((const char *) src0->data + x0 * nb00 + y1 * nb01 +
                                                          i02 * nb02 + i03 * nb03);
                        const float d = *(const float *) ((const char *) src0->data + x1 * nb00 + y1 * nb01 +
                                                          i02 * nb02 + i03 * nb03);
...
static void ggml_compute_forward_flash_attn_ext_f16(const ggml_compute_params * params, const ggml_tensor * q,
                                                    const ggml_tensor * k, const ggml_tensor * v,
                                                    const ggml_tensor * mask, ggml_tensor * dst) {
    ...
    for (int ir = ir0; ir < ir1; ++ir) {
        ...
        float * VKQ32 =
            (float *) params->wdata + ith * (1 * DK + 2 * DV + CACHE_LINE_SIZE_F32);  // FP32 VKQ accumulator
...
static void ggml_flash_attn_ext_qlutattn_f16_segment(const ggml_compute_params * params, const ggml_tensor * q,
                                                     const ggml_tensor * k, const ggml_tensor * v,
                                                     const ggml_tensor * mask, float * dst_data, void * workspace,
    ...
    const int64_t BATCH_SIZE = neq3;  //> batch_size
    ...
    const int64_t STATES_SIZE     = 4 * BATCH_SIZE * N_Q_HEADS * Q_LEN * sizeof(float);  // 2x for quantized and fp32
    const int64_t QKV_BUFFER_SIZE = 3 * DK * sizeof(float);                              // VKQ32, V32 and Q_q

    float * state_data = (float *) ((const uint8_t *) workspace + QLUT_SIZE + QLUT_SCALE_SIZE);  //> Up states buffer.
    float * VKQ32 =
        (float *) ((const uint8_t *) workspace + QLUT_SIZE + QLUT_SCALE_SIZE + STATES_SIZE);     // FP32 VKQ accumulator
    ...
    for (int ir = ir0; ir < ir1; ++ir) {
        ...
        float M = state_data[(iq1 * N_Q_HEADS + iq2) * 2 + 0];
        float S = state_data[(iq1 * N_Q_HEADS + iq2) * 2 + 1];
        ...
        // loop over n_kv and n_head_kv
        // ref: https://arxiv.org/pdf/2112.05682.pdf
        // NOTICE : Loop alone the kv_len dim.
        int standard_valid = 0;
        for (int64_t ic = 0; ic < nek1; ++ic) {
            const float mv = mp ? slope * GGML_FP16_TO_FP32(mp[ic]) : 0.0f;
            if (mv == -INFINITY) {
        ...
        state_data[(iq1 * N_Q_HEADS + iq2) * 2 + 0] = M;
        ...
        // permute(0, 2, 1, 3)
        memcpy((char *) dst_data + (i3 * ne2 * ne1 + i2 + i1 * ne1) * nb1, VKQ32, nb1);
    }
}

static void ggml_flash_attn_ext_qlutattn_segment(const ggml_compute_params * params, const ggml_tensor * q,
    ...
    // const int64_t N_KV_HEAD       = nek0 / (PACK_SIZE * PACK_CHUNK_SIZE);  //> n_kv_head

    //> Q:      [head_dim, q_len,    n_q_head, n_q_batch]
    //> K:      [head_dim * PACK_CHUNK_SIZE, kv_len / PACK_CHUNK_SIZE,   n_q_head, n_q_batch]
    //> V:      [head_dim * PACK_CHUNK_SIZE, kv_len / PACK_CHUNK_SIZE,   n_q_head, n_q_batch]
    //> Mask:   [q_len,    n_q_head, 1,        n_q_batch]

    //> Locate the workspace buffer.
    const uint8_t * pth_qLUT_buffer = (const uint8_t *) workspace;
    float *         state_data      = (float *) ((const uint8_t *) workspace + QLUT_SIZE + QLUT_SCALE_SIZE +
                                    STATES_SIZE / 2);                            //> Down states buffer.
    float *         VKQ32 =
        (float *) ((const uint8_t *) workspace + QLUT_SIZE + QLUT_SCALE_SIZE + STATES_SIZE);  // FP32 VKQ accumulator
    ...
    for (int ir = ir0; ir < ir1; ++ir) {
        ...
        const uint8_t * qLUT_buffer = (const uint8_t *) params->wdata + ir * (QLUT_SIZE + QLUT_SCALE_SIZE);

        //> Change all M/S to ggml_fp16_t
        float       M      = state_data[(iq1 * N_Q_HEADS + iq2) * 2 + 0];
        float       S      = state_data[(iq1 * N_Q_HEADS + iq2) * 2 + 1];
        ggml_fp16_t M_fp16 = ggml_fp32_to_fp16(M);
    ...
    // memcpy(dst_data, VKQ32, PACK_SIZE * sizeof(float));
}

static void ggml_flash_attn_ext_f16_segment(const ggml_compute_params * params, const ggml_tensor * q,
                                            const ggml_tensor * k, const ggml_tensor * v, const ggml_tensor * mask,
                                            float * dst_data, void * workspace, const float scale, const float max_bias,
                                            const float logit_softcap) {
    //> Q:      [head_dim, q_len,    n_q_head, n_q_batch]
    //> K:      [head_dim, kv_len,   n_q_head, n_q_batch]
    //> V:      [head_dim, kv_len,   n_q_head, n_q_batch]
    ...
    for (int ir = ir0; ir < ir1; ++ir) {
        ...
        float * VKQ32 = (float *) workspace + ith * (2 * Q_LEN * N_Q_HEADS + 1 * DK + 2 * DV + CACHE_LINE_SIZE_F32) +
                        2 * Q_LEN * N_Q_HEADS;                   // FP32 VKQ accumulator
...
static void ggml_compute_forward_flash_attn_ext_f16_with_state(const ggml_compute_params * params,
                                                               const ggml_tensor * q, const ggml_tensor * k_fp16,
                                                               const ggml_tensor * v_fp16,
                                                               const ggml_tensor * mask_fp16,
                                                               const ggml_tensor * k_quant, const ggml_tensor * v_quant,
                                                               const ggml_tensor * mask_quant, ggml_tensor * dst) {
    GGML_TENSOR_LOCALS(int64_t, neq, q, ne)
    GGML_TENSOR_LOCALS(size_t, nbq, q, nb)
    GGML_TENSOR_LOCALS(int64_t, ne, dst, ne)
    GGML_TENSOR_LOCALS(size_t, nb, dst, nb)
    ...
    const size_t scratch_per_th = (2 * Q_LEN * N_Q_HEADS + 2 * DV + DK + CACHE_LINE_SIZE_F32);
    ...
    for (int64_t i = 0; i < neq2 * neq1; ++i) {
        ws_state_thread_fp16[2 * i + 0]  = -INFINITY;
        ws_state_thread_fp16[2 * i + 1]  = 0.0f;
    ...
    if (k_quant && v_quant && k_quant->ne[1] > 0) {
        ggml_flash_attn_ext_f16_segment(params, q, k_quant, v_quant, mask_quant, (float *) imm_buffer_quant,
    ...
    for (int64_t ir = ir0; ir < ir1; ++ir) {
        const int64_t iq3 = ir / (neq2 * neq1);     //>  ir / (n_q_head * seq_len)                      => Batch idx.
        const int64_t iq2 =
            (ir - iq3 * neq2 * neq1) / neq1;        //> (ir - iq3 * n_q_head * seq_len) / seq_len       => Head idx.
        const int64_t iq1 =
            (ir - iq3 * neq2 * neq1 - iq2 * neq1);  //> (ir - iq3 * n_q_head * seq_len - iq2*seq_len)   => Sequence idx.

        int64_t state_idx = iq1 * neq2 + iq2;
        float   M_fp16    = ws_state_thread_fp16[2 * state_idx + 0];
        float   S_fp16    = ws_state_thread_fp16[2 * state_idx + 1];
...
void ggml_compute_forward_flash_attn_ext_mixed(const ggml_compute_params * params, const ggml_tensor * q,
                                               const ggml_tensor * k_fp16, const ggml_tensor * v_fp16,
                                               const ggml_tensor * mask_fp16, const ggml_tensor * k_quant,
                                               const ggml_tensor * v_quant, const ggml_tensor * mask_quant,
                                               ggml_tensor * dst) {
    GGML_TENSOR_LOCALS(int64_t, neq, q, ne)
    GGML_TENSOR_LOCALS(size_t, nbq, q, nb)
    GGML_TENSOR_LOCALS(int64_t, ne, dst, ne)
    ...
    const uint8_t * ws_thread_quant  = pth_States_buffer + STATES_SIZE / 2;
    ...
    for (int64_t i = 0; i < N_Q_HEADS * Q_LEN; ++i) {
        ws_state_thread_fp16[2 * i + 0]  = -INFINITY;
        ws_state_thread_fp16[2 * i + 1]  = 0.0f;
        ws_state_thread_quant[2 * i + 0] = -INFINITY;
        ws_state_thread_quant[2 * i + 1] = 0.0f;
...
void ggml_compute_forward_flash_attn_ext(const ggml_compute_params * params, const ggml_tensor * q,
                                         const ggml_tensor * k, const ggml_tensor * v, const ggml_tensor * mask,
                                         const ggml_tensor * k_quant, const ggml_tensor * v_quant, ggml_tensor * dst) {
    switch (dst->op_params[3]) {
        ...
        case GGML_PREC_WITH_STATE:
            {
                ...
                ggml_compute_forward_flash_attn_ext_f16_with_state(params, q, k, v, mask, k_quant, v_quant, mask_quant,
                                                                   dst);
            }
            break;
        case GGML_PREC_F32:
        ...
        case GGML_PREC_MIXED:
            {
                const ggml_tensor * mask_quant = (dst->src[6] != NULL) ? dst->src[6] : NULL;
                ggml_compute_forward_flash_attn_ext_mixed(params, q, k, v, mask, k_quant, v_quant, mask_quant, dst);
            }
            break;
        default:
            {
        ...
        for (int irep = 0; irep < nrep; ++irep) {
            ...
            for (int iq1 = 0; iq1 < neq1; ++iq1) {
                ...
                for (int64_t ic = 0; ic < D; ++ic) {
                    ggml_vec_mad_f32(
                        masked_begin, S, (float *) ((char *) v->data + (ic * nbv1 + iv2 * nbv2 + iv3 * nbv3)),
                ...
                for (int64_t ic = 0; ic < D; ++ic) {
                    ggml_vec_mad_f32(
                        masked_begin, (float *) ((char *) grad_v + (ic * nbgv1 + iv2 * nbgv2 + iv3 * nbgv3)), SM,
                        *(float *) ((char *) d->data + (ic * nbd0 + id1 * nbd1 + id2 * nbd2 + id3 * nbd3)));
                }
            }
        }
