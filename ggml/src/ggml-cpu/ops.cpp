...
static void ggml_compute_forward_flash_attn_ext_f16_with_state(
    ...
    // For this test, we expect that k_quant and v_quant are just additional segments
    // of the KV cache, all should be processed with the standard flash attention
    // This function should process BOTH k,v (FP16) AND k_quant,v_quant together
    
    // DEBUG: Log what we're processing
    printf("[mixed-kv-debug] Processing mixed KV cache:\n");
    printf("[mixed-kv-debug]   k FP16: %s, k_quant: %s\n", 
           k ? "present" : "null", k_quant ? "present" : "null");
    printf("[mixed-kv-debug]   v FP16: %s, v_quant: %s\n", 
           v ? "present" : "null", v_quant ? "present" : "null");
    
    if (k && v && k_quant && v_quant) {
        printf("[mixed-kv-debug]   k FP16: [%ld, %ld, %ld, %ld]\n", k->ne[0], k->ne[1], k->ne[2], k->ne[3]);
        printf("[mixed-kv-debug]   k_quant: [%ld, %ld, %ld, %ld]\n", k_quant->ne[0], k_quant->ne[1], k_quant->ne[2], k_quant->ne[3]);
        printf("[mixed-kv-debug]   v FP16: [%ld, %ld, %ld, %ld]\n", v->ne[0], v->ne[1], v->ne[2], v->ne[3]);
