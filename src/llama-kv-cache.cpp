...
bool llama_kv_cache_unified::find_slot(
    ...
    printf("\n| kvcache n :%-5d | used :%-5d | head :%-5d | n_tokens :%-5d | n_seqs :%-5d | n_seq_tokens :%-5d \n", 
            n, used, head, n_tokens, n_seqs, n_seq_tokens);
...
// ------------------------------------------------------------------
// windowed KV cache: keeps most recent tokens at full precision,
// older tokens can be quantized or evicted to save memory (independent implementation)
// ------------------------------------------------------------------

// Constructor: create internal unified cache and set window_size
llama_kv_cache_windowed::llama_kv_cache_windowed(
        const llama_model & model,
        ggml_type type_k,
        ggml_type type_v,
        bool v_trans,
    ...
    : model(model), hparams(model.hparams), window_size(window_size), 
      v_trans(v_trans), padding(padding), type_k(type_k), type_v(type_v) {
    
    LLAMA_LOG_INFO("%s: creating windowed KV cache, window_size=%u", __func__, window_size);
    ...
    //> Create contexts for allocating memory for the KV cache
    std::map<ggml_backend_buffer_type_t, ggml_context *> ctx_map;
    auto ctx_for_buft = [&](ggml_backend_buffer_type_t buft) -> ggml_context * {
        auto it = ctx_map.find(buft);
        if (it == ctx_map.end()) {
...
void llama_kv_cache_windowed::clear() {
    for (uint32_t i = 0; i < size; ++i) {
        cells[i].pos = -1;
        cells[i].seq_id.clear();
    }
    ...
    LLAMA_LOG_DEBUG("%s: cleared windowed cache", __func__);
}

bool llama_kv_cache_windowed::seq_rm(llama_seq_id seq_id, llama_pos p0, llama_pos p1) {
    uint32_t new_head = size;
...
void llama_kv_cache_windowed::seq_cp(llama_seq_id seq_id_src, llama_seq_id seq_id_dst, llama_pos p0, llama_pos p1) {
    if (seq_id_src == seq_id_dst) {
        return;
    }
...
void llama_kv_cache_windowed::seq_keep(llama_seq_id seq_id) {
    uint32_t new_head = size;
...
void llama_kv_cache_windowed::seq_add(llama_seq_id seq_id, llama_pos p0, llama_pos p1, llama_pos delta) {
    if (delta == 0) {
        return;
    }
...
void llama_kv_cache_windowed::seq_div(llama_seq_id seq_id, llama_pos p0, llama_pos p1, int d) {
    if (d == 1) {
        return;
    }
...
llama_pos llama_kv_cache_windowed::seq_pos_max(llama_seq_id seq_id) const {
    llama_pos result = 0;
...
void llama_kv_cache_windowed::restore() {
    if (pending.ranges.empty()) {
        return;
    }
...
void llama_kv_cache_windowed::commit() {
    if (pending.ranges.empty()) {
        LLAMA_LOG_WARN("%s: no pending KV cache updates to commit - might indicate a bug (ref: %s)\n",
                __func__, "https://github.com/ggml-org/llama.cpp/pull/12695");
        return;
...
// Primary feature of windowed KV cache - evict tokens outside the window
void llama_kv_cache_windowed::evict_old_tokens() {
    if (window_size == 0 || size == 0) {
        return;
    }
    
    // Find the maximum position across all sequences
    llama_pos pos_max = get_pos_max();
    
    // Only keep window_size most recent tokens
    llama_pos window_start = pos_max - (llama_pos)window_size + 1;
    if (window_start <= 0) {
        return; // Window covers all tokens, no need to evict
    }
    
    uint32_t evicted = 0;
    for (uint32_t i = 0; i < size; ++i) {
        // Calculate the actual position with any delta applied
        llama_pos pos = cells[i].pos;
        if (pos >= 0) {
            pos += cells[i].delta;
        }
        
        // If position is before window start, evict the token
        if (pos >= 0 && pos < window_start) {
            cells[i].seq_id.clear();
            cells[i].pos = -1;
            if (used > 0) {
                used--;
    ...
    if (evicted > 0) {
        LLAMA_LOG_DEBUG("%s: evicted %u tokens outside window (before pos %d)\n", 
                       __func__, evicted, window_start);
        
        // Schedule defragmentation since we've created holes in the cache
        do_defrag = true;
    }
}

bool llama_kv_cache_windowed::update(llama_context & lctx) {
    bool need_reserve = false;
    ...
    // Window-specific: evict old tokens
    evict_old_tokens();

    return need_reserve;
}

void llama_kv_cache_windowed::defrag_sched(float thold) {
    // - do not defrag small contexts (i.e. < 2048 tokens)
    // - count the padding towards the number of used tokens
    const float fragmentation = n >= 2048 ? std::max(0.0f, 1.0f - (float(used + padding)/n)) : 0.0f;
...
void llama_kv_cache_windowed::set_full() {
    n = size;
}

llama_sbatch llama_kv_cache_windowed::sbatch_init(
        const llama_batch & batch,
        bool logits_all) {
    return llama_sbatch(batch, hparams.n_embd, true, logits_all);
}

llama_ubatch llama_kv_cache_windowed::ubatch_next(
        llama_sbatch & sbatch,
        uint32_t n_ubatch,
        bool embd_pooled) const {
    GGML_UNUSED(embd_pooled);
...
bool llama_kv_cache_windowed::find_slot(
       const llama_ubatch & ubatch) {
    const uint32_t n_tokens = ubatch.n_tokens;
    const uint32_t n_seqs   = ubatch.n_seqs;
    const uint32_t n_seq_tokens = ubatch.n_seq_tokens;
...
int32_t llama_kv_cache_windowed::get_n_tokens() const {
    int32_t result = 0;
...
int32_t llama_kv_cache_windowed::get_used_cells() const {
    return used;
}

llama_pos llama_kv_cache_windowed::get_pos_max() const {
    llama_pos pos_max = -1;
    for (const auto & cell : cells) {
        pos_max = std::max(pos_max, cell.pos);
    }
...
bool llama_kv_cache_windowed::get_can_shift() const {
    return can_shift;
}

uint32_t llama_kv_cache_windowed::cell_max() const {
    for (uint32_t i = size; i > 0; --i) {
        const kv_cell & cell = cells[i - 1];
...
llm_graph_result_ptr llama_kv_cache_windowed::build_graph_defrag(
        const llama_cparams & cparams,
               ggml_context * ctx,
                ggml_cgraph * gf) const {
    auto res = std::make_unique<llm_graph_result>();
...
size_t llama_kv_cache_windowed::size_k_bytes() const {
    size_t size_k_bytes = 0;
...
size_t llama_kv_cache_windowed::size_v_bytes() const {
    size_t size_v_bytes = 0;
...
bool llama_kv_cache_windowed::defrag_prepare(int32_t n_max_nodes) {
    const uint32_t n_layer = hparams.n_layer;
...
void llama_kv_cache_windowed::state_write(llama_io_write_i & io, llama_seq_id seq_id) const {
    std::vector<std::pair<uint32_t, uint32_t>> cell_ranges; // ranges, from inclusive, to exclusive
    uint32_t cell_count = 0;
...
void llama_kv_cache_windowed::state_read(llama_io_read_i & io, llama_seq_id seq_id) {
    uint32_t cell_count;
    io.read_to(&cell_count, sizeof(cell_count));
...
class llm_graph_input_k_shift_windowed : public llm_graph_input_i {
public:
    llm_graph_input_k_shift_windowed(const llama_kv_cache_windowed * kv_self) : kv_self(kv_self) {}
    virtual ~llm_graph_input_k_shift_windowed() = default;

    void set_input(const llama_ubatch * ubatch) override;

    ggml_tensor * k_shift; // I32 [kv_size]

    const llama_kv_cache_windowed * kv_self;
};

void llm_graph_input_k_shift_windowed::set_input(const llama_ubatch * ubatch) {
    GGML_UNUSED(ubatch);
...
llm_graph_result_ptr llama_kv_cache_windowed::build_graph_shift(
        const llama_cparams & cparams,
               ggml_context * ctx,
                ggml_cgraph * gf) const {
    auto res = std::make_unique<llm_graph_result>();
  ...
  //const auto & n_embd_head_v = hparams.n_embd_head_v;
    ...
    auto inp = std::make_unique<llm_graph_input_k_shift_windowed>(this);
...
ggml_tensor * llama_kv_cache_windowed::build_rope_shift(
        const llama_cparams & cparams,
               ggml_context * ctx,
                ggml_tensor * cur,
                ggml_tensor * shift,
...
void llama_kv_cache_windowed::state_write_meta(
        llama_io_write_i & io, 
...
void llama_kv_cache_windowed::state_write_data(
        llama_io_write_i & io, 
...
bool llama_kv_cache_windowed::state_read_meta(llama_io_read_i & io, uint32_t cell_count, llama_seq_id dest_seq_id) {
    if (dest_seq_id != -1) {
        // single sequence
...
bool llama_kv_cache_windowed::state_read_data(llama_io_read_i & io, uint32_t cell_count) {
    uint32_t v_trans;
    uint32_t n_layer;
    io.read_to(&v_trans, sizeof(v_trans));
    io.read_to(&n_layer, sizeof(n_layer));
...
llama_kv_cache * llama_kv_cache_windowed_create(
        ...
        uint32_t padding,
        uint32_t window_size) {
    return new llama_kv_cache_windowed(
        model, type_k, type_v, v_trans, offload,
        use_extra_buft, kv_size, padding, window_size);
}
