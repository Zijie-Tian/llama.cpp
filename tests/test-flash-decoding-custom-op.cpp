...
torch::Tensor torch_flash_attention(torch::Tensor Q, torch::Tensor K, torch::Tensor V,
                                    torch::Tensor mask = torch::Tensor(), float scale = 1.0f) {
...
void ggml_custom_flash_attn_mixed_simple(ggml_tensor * dst, int ith, int nth, void * wdata, size_t wsize,
                                         void * userdata);

// Parameters for flash attention are defined in llama-kv-cache-mixed.h
static void fill_random_f32(ggml_tensor * dst, size_t n_rows, size_t n_cols, float min_val = -1.0f,
                            float max_val = 1.0f) {
...
int main() {
    ...
    //> [n_kv, seq_len, 1, 1]
    ggml_tensor * mask =
        ggml_new_tensor_2d(ctx, GGML_TYPE_F32, GGML_PAD(kv_len, n_pad), GGML_PAD(seq_len, GGML_KQ_MASK_PAD));
    ...
    if (quant_len > 0) {
        ...
        ggml_tensor * k_quant_src = ggml_new_tensor_4d(ctx, GGML_TYPE_F16, head_dim, quant_len, n_kv_heads, 1);
        ggml_tensor * v_quant_src = ggml_new_tensor_4d(ctx, GGML_TYPE_F16, head_dim, quant_len, n_kv_heads, 1);

        k_quant = ggml_new_tensor_4d(ctx, GGML_TYPE_F16, head_dim, quant_len, n_kv_heads, 1);
        v_quant = ggml_new_tensor_4d(ctx, GGML_TYPE_F16, head_dim, quant_len, n_kv_heads, 1);
        ...
        for (size_t h = 0; h < n_kv_heads; h++) {
            for (size_t t = 0; t < quant_len; t++) {
                for (size_t d = 0; d < head_dim; d++) {
                    ...
                    size_t dst_idx      = d + t * head_dim + h * head_dim * quant_len;
        ...
        printf("K_quant final shape: [%zu, %zu, %zu, %zu] type: %s\n", k_quant->ne[0], k_quant->ne[1], k_quant->ne[2],
               k_quant->ne[3], ggml_type_name(k_quant->type));
        printf("V_quant final shape: [%zu, %zu, %zu, %zu] type: %s\n", v_quant->ne[0], v_quant->ne[1], v_quant->ne[2],
               v_quant->ne[3], ggml_type_name(v_quant->type));
    ...
    ggml_tensor * custom_result =
        ggml_flash_attn_mixed(ctx, q, k_fp16, v_fp16, mask, k_quant, v_quant, mask,  // Use NULL mask for comparison
                              1 / std::sqrt(head_dim),
                              0.0f,                                                  // max_bias
                              0.0f                                                   // logit_softcap
        );
    ...
    const size_t elements_per_thread        = output_size + local_max_size + local_sum_size + 2 * temp_buffer_size +
                                       q_quantized_float_elements + 1 +
                                       16;  // +1 for sync_buffer, +16 for CACHE_LINE_SIZE_F32

    struct ggml_threadpool_params * tp_params =
        (struct ggml_threadpool_params *) malloc(sizeof(struct ggml_threadpool_params));
    ...
    ggml_tensor * mask_std =
        ggml_new_tensor_2d(ctx, GGML_TYPE_F16, GGML_PAD(kv_len, n_pad), GGML_PAD(seq_len, GGML_KQ_MASK_PAD));
    ...
    for (int64_t q_pos = 0; q_pos < mask_std->ne[1]; q_pos++) {
        for (int64_t kv_pos = 0; kv_pos < mask_std->ne[0]; kv_pos++) {
            mask_std_data[q_pos * mask_std->ne[0] + kv_pos] =
                (ggml_fp16_t) ggml_fp32_to_fp16(mask_data[q_pos * mask->ne[0] + kv_pos]);
        }
    }
    ...
    ggml_tensor * standard_result = ggml_flash_attn_ext(ctx, q_std, k, v, mask_std,  // Use NULL mask for comparison
                                                        scale,
                                                        0.0f,                        // max_bias
                                                        0.0f                         // logit_softcap
    );
    ...
    printf("Standard result shape: [%ld, %ld, %ld, %ld]\n", standard_result->ne[0], standard_result->ne[1],
           standard_result->ne[2], standard_result->ne[3]);
    ...
    try {
        ...
        auto  torch_result = torch::scaled_dot_product_attention(q_torch, k_torch, v_torch, mask_torch,
                                                                 /*dropout_p=*/0.0,
                                                                 /*is_causal=*/false,
                                                                 /*scale=*/scale_factor);
        torch_result       = torch_result.permute({ 0, 2, 1, 3 }).contiguous();  //> [1, seq_len, n_heads, head_dim]

        printf("PyTorch result shape: [%ld, %ld, %ld, %ld]\n", torch_result.size(0), torch_result.size(1),
               torch_result.size(2), torch_result.size(3));
        ...
        if (torch_success) {
            ...
            if (std::isfinite(custom_val) && std::isfinite(standard_val) && std::isfinite(torch_val)) {
                ...
                printf("%5zu | %11.6f | %11.6f | %11.6f | %.6e | %.6e | %.6e\n", i, custom_val, standard_val, torch_val,
                       abs_diff_cs, abs_diff_ct, abs_diff_st);
            } else {
                printf("%5zu | %11.6f | %11.6f | %11.6f |     N/A     |     N/A     |     N/A\n", i, custom_val,
                       standard_val, torch_val);
            }
        } else {
            if (std::isfinite(custom_val) && std::isfinite(standard_val)) {
                float abs_diff_cs = std::abs(custom_val - standard_val);
    ...
    printf("  Custom vs Standard: %s (max diff: %.6e)\n",
           (compared_elements > 0 && max_custom_standard < tolerance) ? "PASS" : "FAIL", max_custom_standard);

    if (torch_success) {
        printf("  Custom vs PyTorch: %s (max diff: %.6e)\n",
               (compared_elements > 0 && max_custom_torch < tolerance) ? "PASS" : "FAIL", max_custom_torch);
        printf("  Standard vs PyTorch: %s (max diff: %.6e)\n",
               (compared_elements > 0 && max_standard_torch < tolerance) ? "PASS" : "FAIL", max_standard_torch);
    } else {
