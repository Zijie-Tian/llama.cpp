...
static void calculate_error_metrics(const float * original, const float * reconstructed, size_t n, float & max_error,
                                    float & avg_error, float & rms_error, float & max_rel_error,
                                    float & avg_rel_error) {
    max_error     = 0.0f;
    avg_error     = 0.0f;
    rms_error     = 0.0f;
...
static float calculate_snr(const float * original, const float * reconstructed, size_t n) {
    ...
    if (noise_power < 1e-10f) {
        return 100.0f;  // arbitrary high value for near-zero noise
    }
...
static void print_csv_header() {
    printf(
        "type,bits_per_val,compression_ratio,max_abs_error,avg_abs_error,rms_error,max_rel_error_percent,avg_rel_error_"
        "percent,snr_db\n");
}

static void run_test_for_type(ggml_type type, const float * input_data, float * quantized_data, float * output_data,
                              size_t test_size, bool verbose, bool csv_output) {
    const auto * qfns     = ggml_get_type_traits(type);
    const auto * qfns_cpu = ggml_get_type_traits_cpu(type);
    ...
    calculate_error_metrics(input_data, output_data, test_size, max_error, avg_error, rms_error, max_rel_error,
                            avg_rel_error);
    ...
    if (csv_output) {
        ...
        printf("%s,%.2f,%.2f,%.6f,%.6f,%.6f,%.6f,%.6f,%.2f\n", ggml_type_name(type), bits_per_val, compression_ratio,
               max_error, avg_error, rms_error, max_rel_error * 100.0f, avg_rel_error * 100.0f, snr);
    } else {
        ...
        if (verbose) {
            ...
            for (size_t j = 0; j < std::min(test_size, size_t(20)); j++) {
                printf("[%4zu] %.6f -> %.6f (error: %.6f)\n", j, input_data[j], output_data[j],
                       fabsf(input_data[j] - output_data[j]));
            }
...
int main(int argc, char * argv[]) {
    ...
    for (int i = 0; i < GGML_TYPE_COUNT; i++) {
        ...
        if (!params.include_types.empty() && ggml_type_name(type) &&
            std::find(params.include_types.begin(), params.include_types.end(), ggml_type_name(type)) ==
                params.include_types.end()) {
            // printf("skipping %s due to NOT in include_types.\n", ggml_type_name(type));
            continue;
        }

        if (qfns_cpu->from_float && qfns->to_float) {
            run_test_for_type(type, input_data, quantized_data, output_data, params.test_size, params.verbose,
                              params.csv_output);
        } else {
