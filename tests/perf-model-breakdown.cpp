...
struct test_case
{
    ...
    //> perf without op_name.
    bool eval_perf(ggml_backend_t backend)
    {
        mode = MODE_PERF;
...
enum llama_model_type {
    LLAMA_MODEL_TYPE_LLAMA,      // llama-2-7b
    LLAMA_MODEL_TYPE_BAICHUAN,
    LLAMA_MODEL_TYPE_STARCODER,
};

void get_llama_hparams(llama_model_type model_type, llama_hparams &hparams) {
    switch (model_type) {
        case LLAMA_MODEL_TYPE_LLAMA:    // llama-2-7b
            hparams = {
                /*n_vocab        =*/32000,
                /*n_embd         =*/4096,
                /*n_head         =*/32,
            ...
            break;
        default:
            GGML_ABORT("invalid model type");
    }
}

static bool perf_llm_breakdown(ggml_backend_t backend, llama_model_type model_type, ggml_type type_w, ggml_type type_a, int64_t n_batch) {
    llama_hparams hparams;
    get_llama_hparams(model_type, hparams);

    std::vector<std::unique_ptr<test_case>> test_cases;

    //> Q/K/V projection
    test_cases.emplace_back(new test_mul_mat(type_w, type_a, hparams.n_embd, 1, hparams.n_embd, {n_batch,  1}, {1, 1}));
    test_cases.emplace_back(new test_mul_mat(type_w, type_a, hparams.n_embd, 1, hparams.n_embd, {n_batch,  1}, {1, 1}));
    test_cases.emplace_back(new test_mul_mat(type_w, type_a, hparams.n_embd, 1, hparams.n_embd, {n_batch,  1}, {1, 1}));

    //> ROPE
    test_cases.emplace_back(new test_rope(type_a, {hparams.n_embd_head, hparams.n_head, hparams.n_tokens, n_batch}, 1, 0, 512, 1.0f, 0.0f, 0.0f, false, 0));
    test_cases.emplace_back(new test_rope(type_a, {hparams.n_embd_head, hparams.n_head, hparams.n_tokens, n_batch}, 1, 0, 512, 1.0f, 0.0f, 0.0f, false, 0));

    //> Attention
    test_cases.emplace_back(new test_flash_attn_ext(hparams.n_embd_head, hparams.n_head, 4096 * n_batch, 1, true, 0.0f, type_a));
    test_cases.emplace_back(new test_mul_mat(type_w, type_a, hparams.n_embd, 1, hparams.n_embd, {n_batch, 1}, {1, 1}));

    //> Feed-forward network
    test_cases.emplace_back(new test_mul_mat(type_w, type_a, hparams.n_ff, 1, hparams.n_embd, {n_batch, 1}, {1, 1}));
    test_cases.emplace_back(new test_mul_mat(type_w, type_a, hparams.n_ff, 1, hparams.n_embd, {n_batch, 1}, {1, 1}));
    test_cases.emplace_back(new test_mul_mat(type_w, type_a, hparams.n_embd, 1, hparams.n_ff, {n_batch, 1}, {1, 1}));

    for (auto &test : test_cases) {
        test->eval_perf(backend);
    }
    return true;
...
int main(int argc, char **argv)
{
    ...
    for (size_t i = 0; i < ggml_backend_reg_get_count(); i++)
    {
        ...
        bool ok = perf_llm_breakdown(backend, LLAMA_MODEL_TYPE_LLAMA, GGML_TYPE_Q4_0, GGML_TYPE_F32, n_batch);
