...
template <typename T> static void fillQ4blocks(std::vector<T> & blocks, std::mt19937 & rndm) {
    for (auto & b : blocks) {
        b.d = 1;
...
int main(int argc, char ** argv) {
    ...
    for (int iloop = 0; iloop < nloop; ++iloop) {
        ...
        float fs;
        if (type == 0) {
            funcs->vec_dot(kVecSize * QK4_1, &fs, 0, x40.data(), 0, y.data(), 0, 1);
        } else {
            funcs->vec_dot(kVecSize * QK4_1, &fs, 0, x41.data(), 0, y.data(), 0, 1);
        }
