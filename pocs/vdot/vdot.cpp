...
inline double dot(int n, const block_q4_0 * x, const float * y) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int k = 0; k < 4; ++k) {
            u1 = u[k] & kMask1;
            u2 = (u[k] >> 4) & kMask1;
            s += y[0] * kValues[q1[0]] + y[1] * kValues[q2[0]] + y[2] * kValues[q1[1]] + y[3] * kValues[q2[1]] +
                 y[4] * kValues[q1[2]] + y[5] * kValues[q2[2]] + y[6] * kValues[q1[3]] + y[7] * kValues[q2[3]];
            y += 8;
        }
        sum += s * d;
        ++x;
...
inline double dot3(int n, const block_q4_0 * x, const float * y) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        float s = 0;
        for (int k = 0; k < 4; ++k) {
            s += y[0] * kValues[q[0]].first + y[1] * kValues[q[0]].second + y[2] * kValues[q[1]].first +
                 y[3] * kValues[q[1]].second + y[4] * kValues[q[2]].first + y[5] * kValues[q[2]].second +
...
inline double dot41(int n, const block_q4_1 * x, const float * y) {
    const static float kValues[16] = { 0.f, 1.f, 2.f,  3.f,  4.f,  5.f,  6.f,  7.f,
...
int main(int argc, char ** argv) {
    ...
    for (int iloop = 0; iloop < nloop; ++iloop) {
        ...
        if (scalar) {
            ...
            if (useQ4_1) {
                funcs_cpu->vec_dot(kVecSize, &result, 0, q41.data(), 0, q8.data(), 0, 1);
            } else {
                funcs_cpu->vec_dot(kVecSize, &result, 0, q40.data(), 0, q8.data(), 0, 1);
            }
        }
        sumq += result;
