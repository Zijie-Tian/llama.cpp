    ...
    for (int64_t b = 0; b < batch; b++) {
        for (int64_t q = 0; q < q_len_padded; q++) {
            for (int64_t kv = 0; kv < kv_len; kv++) {
                size_t idx = b * mask->nb[3]/sizeof(float) + 
                            q * mask->nb[1]/sizeof(float) + 
...
int main() {
    ...
    // Query tensor
    ggml_tensor* Q = ggml_new_tensor_4d(ctx, GGML_TYPE_F16, 
        cfg.head_dim, cfg.q_len, cfg.n_heads, cfg.batch_size);
    ggml_set_name(Q, "Q");
    
    // KV tensors
    ggml_tensor* K = ggml_new_tensor_4d(ctx, GGML_TYPE_F16,
        cfg.head_dim, cfg.kv_len, cfg.n_heads, cfg.batch_size);
    ggml_set_name(K, "K");
    
    ggml_tensor* V = ggml_new_tensor_4d(ctx, GGML_TYPE_F16,
        cfg.head_dim, cfg.kv_len, cfg.n_heads, cfg.batch_size);
    ggml_set_name(V, "V");
    ...
    ggml_tensor* mask = ggml_new_tensor_4d(ctx, GGML_TYPE_F32,
        cfg.kv_len, q_len_padded, 1, cfg.batch_size);
    ggml_set_name(mask, "mask");
    
    // For with_state version, split KV into two parts
    int64_t kv_split = cfg.kv_len / 2;
    
    ggml_tensor* K_part1 = ggml_new_tensor_4d(ctx, GGML_TYPE_F16,
        cfg.head_dim, kv_split, cfg.n_heads, cfg.batch_size);
    ggml_set_name(K_part1, "K_part1");
    
    ggml_tensor* V_part1 = ggml_new_tensor_4d(ctx, GGML_TYPE_F16,
        cfg.head_dim, kv_split, cfg.n_heads, cfg.batch_size);
    ggml_set_name(V_part1, "V_part1");
    
    ggml_tensor* K_part2 = ggml_new_tensor_4d(ctx, GGML_TYPE_F16,
        cfg.head_dim, kv_split, cfg.n_heads, cfg.batch_size);
    ggml_set_name(K_part2, "K_part2");
    
    ggml_tensor* V_part2 = ggml_new_tensor_4d(ctx, GGML_TYPE_F16,
        cfg.head_dim, kv_split, cfg.n_heads, cfg.batch_size);
    ggml_set_name(V_part2, "V_part2");
    
    ggml_tensor* mask_part1 = ggml_new_tensor_4d(ctx, GGML_TYPE_F32,
        kv_split, q_len_padded, 1, cfg.batch_size);
    ggml_set_name(mask_part1, "mask_part1");
    
    ggml_tensor* mask_part2 = ggml_new_tensor_4d(ctx, GGML_TYPE_F32,
        kv_split, q_len_padded, 1, cfg.batch_size);
    ...
    for (int64_t b = 0; b < cfg.batch_size; b++) {
        for (int64_t q = 0; q < q_len_padded; q++) {
            ...
            for (int64_t kv = 0; kv < kv_split; kv++) {
                size_t src_idx = b * (mask->nb[3]/sizeof(float)) + 
                                q * (mask->nb[1]/sizeof(float)) + 
                                kv * (mask->nb[0]/sizeof(float));
                size_t dst_idx = b * (mask_part1->nb[3]/sizeof(float)) + 
                                q * (mask_part1->nb[1]/sizeof(float)) + 
            ...
            for (int64_t kv = 0; kv < kv_split; kv++) {
                ...
                size_t dst_idx = b * (mask_part2->nb[3]/sizeof(float)) + 
                                q * (mask_part2->nb[1]/sizeof(float)) + 
    ...
    ggml_tensor* result_std = ggml_flash_attn_ext(
    ...
    ggml_tensor* result_state = ggml_flash_attn_ext_with_state(
        ctx, Q, K_part2, V_part2, mask_part2, K_part1, V_part1, mask_part1, scale, 0.0f, 0.0f
    );
