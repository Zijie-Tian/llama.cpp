...
static void quantize_symmetric(const float * input, uint8_t * output, float * scales, 
    ...
    for (int g = 0; g < n_groups; g++) {
        ...
        if (scales[g] < 1e-5f) scales[g] = 1e-5f;
...
public:
    void benchmark_comparison(int M, int K, int N = 1) {
        ...
        // ===================================================================================================
        {
            ...
            for (int iter = 0; iter < n_warmup; iter++) {
                ...
                for (int n = 0; n < N; n++) {
                    ...
                    ggml::cpu::qlutattn::qlutattn_lut_ctor_int8_g4(
                        B, lut_scales, lut_biases, qlut, K, kernel_config
                    );
                }
                
                memset(output, 0, M * N * sizeof(test_float_type));
                ggml::cpu::qlutattn::qgemm_lut_int8_g4(
                    quantized_weights, QLUT, scales, LUT_Scales, LUT_Biases,
                    output, kernel_config->bm, K, N, kernel_config
                );
            }
            ...
            // Memory bandwidth: weights + activation + output
            double bytes = (M * K * bits / 8.0) + (N * K * 2) + (M * N * 2);  // 2 bytes for FP16
            result.throughput_gbps = (bytes / 1e9) / (result.mean_ms / 1000.0);
        ...
        benchmark_q40:
        {
            ...
            int nb = (M * K) / QK4_0;
            block_q4_0 * q4_weights = (block_q4_0 *) aligned_malloc(nb * sizeof(block_q4_0));
            ...
            for (int iter = 0; iter < n_warmup; iter++) {
                for (int m = 0; m < M; m++) {
                    for (int n = 0; n < N; n++) {
                        funcs->vec_dot(K, &output_q40[m * N + n], 0, 
                                      q4_weights + m * K / QK4_0, 0, 
                                      activation_f32 + n * K, 0, 1);
                    }
                }
            }
            ...
            // Memory bandwidth: Q4_0 weights + FP32 activation + FP32 output
            double bytes = (nb * sizeof(block_q4_0)) + (N * K * 4) + (M * N * 4);
            result.throughput_gbps = (bytes / 1e9) / (result.mean_ms / 1000.0);
        ...
        // ===================================================================================================
        {
            ...
            // Memory bandwidth: FP32 weights + FP32 activation + FP32 output
            double bytes = (M * K * 4) + (N * K * 4) + (M * N * 4);
            result.throughput_gbps = (bytes / 1e9) / (result.mean_ms / 1000.0);
    ...
    void run_benchmark_suite() {
        ...
        for (const auto& test : test_cases) {
            try {
                ...
                size_t memory_needed = (size_t)test.M * test.K * sizeof(float) * 4 + 
                                      (size_t)test.M * test.N * sizeof(float) * 4;
        ...
        // Print summary
        printf("\nSummary:\n");
        printf("- QLUTATTN uses LUT-based computation with 4-bit quantization\n");
        ...
        printf("- Lower time and higher throughput indicate better performance\n");
    }
};
